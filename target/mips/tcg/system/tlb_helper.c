...
void mips_cpu_do_interrupt(CPUState *cs)
{
 ...
 enter_debug_mode:
        ...
        if (!(env->CP0_Status & (1 << CP0St_EXL))) {
            cpu_mips_update_cause(env, 1U << CP0Ca_BD, 0);
        }
 ...
 set_error_EPC:
    ...
    case EXCP_CpU:
        cause = 11;
        update_badinstr = 1;
        cpu_mips_update_cause(env, 0x3 << CP0Ca_CE,
 ...
 set_EPC:
        if (!(env->CP0_Status & (1 << CP0St_EXL))) {
            ...
            cpu_mips_update_cause(env, 1U << CP0Ca_BD,
                                  env->hflags & MIPS_HFLAG_BMASK ?
                                  1U << CP0Ca_BD : 0);
            env->CP0_Status |= (1 << CP0St_EXL);
            if (env->insn_flags & ISA_MIPS3) {
        ...
        set_hflags_for_handler(env);
        cpu_mips_update_cause(env, 0x1f << CP0Ca_EC, cause << CP0Ca_EC);
        break;
    default:
        abort();
    }
